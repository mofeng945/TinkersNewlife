...
public final class CurseVaultHudRenderer {
    ...
    public static void render(ForgeGui gui, GuiGraphics graphics, float partialTick, int width, int height) {
        ...
        if (!(be instanceof CurseVaultBlockEntity vault)) return;

        double power = vault.getClientPower();
        Component text = Component.translatable("hud.tinkersnewlife.curse_vault",
        ...
        int filled = (int) Math.round(barW * Math.max(0.0, Math.min(1.0, power / CurseVaultData.CAPACITY)));
        if (filled > 0) {
