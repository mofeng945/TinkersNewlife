...
public final class IronSpellsFluids {
     ...
     *
     * <p>与雷电瓶（{@code irons_spellbooks:lightning_bottle}，"对闪电苦力怕使用玻璃瓶"得到）
     * 双向互转，<b>250 mB = 1 瓶</b>（与匠魂默认的"一瓶龙血 250 mB"同一规格）✓：
     * <pre>
     *   接取：玻璃瓶 + 250 mB 液态闪电 → 雷电瓶（mantle:fill_item / 浇筑台皆可）
     *   倒出：雷电瓶 → 250 mB 液态闪电 + 玻璃瓶（排液口 / 浇口）
     * </pre>
     * 两个方向严格 1:1 ✓ —— 既不会凭空多出瓶子，也不会把瓶子倒没 ✓，
     * 所以"玻璃瓶 ⇄ 液态闪电"本身就是一个自洽的闭环，不需要额外的产出源 ✓。
     *
     * <p>放在<b>铁魔法联动组</b>（与雷电瓶同生共死：没装铁魔法 → 这一支连同它的桶、
     * 液体方块、三条配方一起不注册/不加载 ✓）。
     */
