 ...
 *       小地图雷达也显示不出你 —— 这一条只能靠<b>隐身标记</b>实现
 *       （Xaero's 之类只认 {@code isInvisible()}，没有任何"单独屏蔽某个可见玩家"的接口 ✗），
 *       所以佩戴者会处于隐身态（身体不可见，盔甲/手持物照常显示）；
 *       不想要隐身态就用配置 {@code cognitive_mask.invisible=false} 关掉
 *       （名字与锁定仍然照旧屏蔽，但雷达会照常显示你）。</li>
