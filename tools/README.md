...
### 星象仪**图标选择**核对（§621 靠它抓到"满月图标永不可达"）

| 文件 | 干什么 |
| --- | --- |
| `VerifyPlanetariumIcons.java` | 读模型 JSON（base 层的 `layer1` + 7 条 override 的阈值），按原版选取规则**逐相位枚举**算出命中哪张图，并统计"有几张图真的能被命中"。⚠ 以后凡是"N 档切换"的机制（模型 overrides / 属性谓词 / 资源优先级），都先用它这种"枚举输入 + 看落点 + 看可达性"的方式验一遍 |