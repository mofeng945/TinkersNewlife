...
- The **Durandal sword** sprite was a 256×256 painted image; it is now a **64×64 pixel-art sprite缩绘 from that same art** — the silhouette (44° diagonal, spiked cross guard, flame-edged blade) is taken straight from the original, not redrawn.
- The downscale is a small image pipeline (`tools/downscale-durandal-sword.ps1`): **max-alpha mask** (so thin spikes survive instead of breaking into dashes), colour sampled only from the art's *solid* pixels, a 1px **hole fill**, a **1px thickening**, and tone quantisation. `$SIZE = 32` gives the same sword at 32×32; `$MASK_THRESHOLD` / `$DILATE` / `$GAIN` tune thickness and darkness.
- The 256×256 original is kept in the repo (`tools/art-src/`) so the sprite can be regenerated at any resolution.
